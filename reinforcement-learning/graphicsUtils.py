...
_root_window: tkinter.Tk = None  # The root window for graphics output
_canvas: tkinter.Canvas = None  # The canvas which holds graphics
...
def formatColor(red, green, blue):
...
def begin_graphics(width=640, height=480, color=formatColor(0, 0, 0), title=None):
    ...
    _root_window = tkinter.Tk()  # done at the top, line 21 (as of 2nd May)
...
def line(here, there, color=formatColor(0, 0, 0), width=2, tag_raise=0):
    x0, y0 = here[0], here[1]
    x1, y1 = there[0], there[1]
    line_ = _canvas.create_line(x0, y0, x1, y1, fill=color, width=width)
    if tag_raise > 0:
        _canvas.tag_lower(line_, tag_raise)
    return line_
...
def keys_pressed(d_o_e=lambda arg: _root_window.tk.dooneevent(arg), d_w=tkinter._tkinter.DONT_WAIT):
    d_o_e(d_w)
    if _got_release:
        d_o_e(d_w)
...
def remove_from_screen(x, d_o_e=lambda arg: _root_window.tk.dooneevent(arg), d_w=tkinter._tkinter.DONT_WAIT):
    _canvas.delete(x)
    d_o_e(d_w)
...
def move_to(object, x, y=None, d_o_e=lambda arg: _root_window.tk.dooneevent(arg), d_w=tkinter._tkinter.DONT_WAIT):
    if y is None:
        try:
            x, y = x
        except:
